...
def show_update_detail():
    ...
    if request.method=='POST':
        ...
        qry_pat = "select Patient_ID, bloodtype_req, reason_of_req, Doctor_name from Patient inner join Doctor on Doctor.Doctor_ID = Patient.Doctor_ID and User_ID = %s" %(request.form['User_ID'])
        qry_don = "select Donor_ID, blood_donated, reason_of_donation, Organization_name from Donor inner join Organization on Organization.Organization_ID = Donor.Organization_ID and User_ID = %s" %(request.form['User_ID'])
        qry_trans = "select distinct Transaction.Patient_ID, Transaction.Donor_ID, Blood_ID, Date_of_transaction, Status from Transaction, Patient, Donor where (Patient.User_ID = %s and Patient.Patient_ID = Transaction.Patient_ID) or (Donor.User_Id= %s and Donor.Donor_ID = Transaction.Donor_ID)" %((request.form['User_ID']),(request.form['User_ID']))
        #
        res_pat = ()
        res_dnr = ()
        res_trans = ()
...
def add_Donor():
    ...
    qry_insert = "insert into blood_available (blood_type, Donor_ID) Values (%s,%s) "%(val[1],val[0])
...
def add_Doctor():
    ...
    qry = "INSERT INTO Doctor Values (%s,%s,%s)"%val
    print(qry)
    success = True
    error = False
    try:
...
@app.route("/add_blood_available", methods=['POST','GET'])
def add_blood_available():
    if not session.get('login'):
        return redirect( url_for('home') )
    qry = "SELECT * from blood_available"
    mycursor.execute(qry)
    fields = mycursor.column_names
    ...
    qry = "INSERT INTO blood_available Values (%s,%s,%s)"%val
    print(qry)
    success = True
    error = False
    try:
    ...
    return redirect(url_for('add_page', id='blood_available', error=error,success=success))
...
def add_Transaction_head():
    ...
    for field in fields:
        temp = request.form.get(field)
        if field not in ['Patient_ID','Donor_ID','Status','Blood_ID'] and temp != '':
            temp = "\'"+temp+"\'"
        if temp == '':
            temp = 'NULL'
        val = val + (temp,)
    ...
    qry_insert = "delete from blood_available where Blood_ID = %s "%val[1]
...
def update_patient_details():
    ...
    for field in fields:
        if request.form[field] not in ['None','']:
            ...
            qry = qry + "%s = NULL , " %(field)
    qry = qry[:-2]
    qry = qry + "WHERE Patient_ID = %s and bloodtype_req = \'%s\';" %(request.form['Patient_ID'],request.form['bloodtype_req'])
    print(qry)
    try:
        mycursor.execute(qry)
    except:
        print("update error")
    mydb.commit()
    qry2 = "select * from Patient WHERE Patient_ID = %s and bloodtype_req = \'%s\';" %(request.form['Patient_ID'],request.form['bloodtype_req'])
    mycursor.execute(qry2)
    res = mycursor.fetchone()
    print(res)
    print(qry2)
...
def update_donor_details():
    ...
    for field in fields:
        if request.form[field] not in ['None','']:
            ...
            qry = qry + "%s = NULL , " %(field)
    qry = qry[:-2]
    qry = qry + "WHERE Donor_ID = %s and blood_donated = \"%s\";" %(request.form['Donor_ID'],request.form['blood_donated'])
    print(qry)
    try:
        mycursor.execute(qry)
    except:
        print("update error")
    mydb.commit()
    qry2 = "select * from Patient WHERE Blood_ID = %s and blood_donated = \"%s\";" %(request.form['Donor_ID'],request.form['blood_donated'])
    mycursor.execute(qry2)
    res = mycursor.fetchone()
    print(res)
    print(qry2)
...
def del_patient():
    if not session.get('login'):
        return redirect( url_for('home') )
    qry = "delete from Patient where Patient_ID="+str(request.form['Patient_ID'])+" and bloodtype_req=\'%s\'"%(request.form['bloodtype_req'])
    print(qry)
    try:
        mycursor.execute(qry)
    except:
...
def del_donor():
    if not session.get('login'):
        return redirect( url_for('home') )
    qry = "delete from Donor where Donor_ID="+str(request.form['Donor_ID'])+" and blood_donated=\'%s\'" %request.form['blood_donated']
    try:
        mycursor.execute(qry)
    except:
        print("Error in deletion")
...
def ststats(blood_list=None):
    if not session.get('login') or not session.get('isAdmin'):
        return redirect( url_for('home') )
    qry = "select blood_donated, count(Donor_ID) from Donor group by blood_donated"
    mycursor.execute(qry)
    stats_donor = mycursor.fetchall()
    A = []
    B = []
    for blood in stats_donor:
        A.append(blood[0])
        B.append(blood[1])
    plt.pie(B, labels = A)
    ...
    A.clear()
    B.clear()
    qry = "select bloodtype_req, count(Patient_Id) from Patient group by bloodtype_req"
    mycursor.execute(qry)
    ...
    # plt.show()
    plt.close()
    qry = "select distinct blood_donated from Transaction inner join Donor on Transaction.Donor_ID = Donor.Donor_ID"
    mycursor.execute(qry)
    list = mycursor.fetchall()
    blood_list = []
    for blood in list:
        print(blood)
        blood_list.append(blood[0])
    print(blood)
    A.clear()
    B.clear()
    for blood in blood_list:
        qry = "select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where blood_donated = '%s' and Status = 1" %blood
        print(qry)
        mycursor.execute(qry)
        a = mycursor.fetchone()
        A.append(a[0])
        qry = "select count(*) from Transaction inner join Donor on Donor.Donor_ID = Transaction.Donor_ID where blood_donated = '%s' and Status = 0" %blood
        print(qry)
        mycursor.execute(qry)
        b = mycursor.fetchone()
        B.append(b[0])
    ...
    plt.xlabel('Blood')
    plt.title('SUCCESS V/S FAILURE IN BLOOD TRANSPLANTATION')
    plt.xticks(ind + width / 2, blood_list)
    plt.legend(loc='best')
